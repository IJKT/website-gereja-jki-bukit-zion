<x-layout_sistem_informasi>
    ...
    <div class="flex-1 bg-white p-10">
        <form id="pujianForm" action="{{ route('Jadwal.UpdateLagu') }}" method="POST">
            @csrf
            @method('PUT')
            <div class="bg-gray-200 p-6 rounded-md">
                ...
                <div class="grid grid-cols-2 gap-x-10 gap-y-4">
                    <div>
                        ...
                        <input name="id_jadwal" type="text" value="{{ $data_lagu->id_jadwal }}"
                            class="w-full p-2 rounded bg-gray-100" readonly>
                    </div>
                    <div class="relative">
                        <label class="block font-semibold mb-1">JUDUL LAGU</label>
                        <input type="text" id="nama_lagu" name="nama_lagu"
                            value="{{ $data_lagu->lagu_pujian->nama_lagu }}" class="w-full p-2 rounded bg-gray-100"
                            placeholder="Tambahkan Judul Lagu" autocomplete="off" readonly>
                        ...
                        <input type="hidden" id="id_lagu" name="id_lagu" value="{{ $data_lagu->id_lagu }}"
                            required />
                    </div>
                    <div>
                        <label class="block font-semibold mb-1">URUTAN LAGU</label>
                        <input name="urutan_lagu" type="text" value="{{ $data_lagu->urutan_lagu }}"
                            inputmode="numeric" class="w-full p-2 rounded bg-white" required>
                    </div>
                </div>
            </div>
        </form>
        ...
        <div class="fixed bottom-0 right-0 mb-4 mr-4 text-white font-bold">
            <a href="{{ route('Jadwal.viewall_pujian', $data_lagu->id_jadwal) }}">
