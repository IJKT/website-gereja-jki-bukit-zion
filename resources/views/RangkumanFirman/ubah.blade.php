<x-layout_sistem_informasi>
    ...
    <div class="flex-1 bg-white p-10">
        <form id="rangkumanForm" action="{{ route('RangkumanFirman.update', $rangkuman) }}" method="POST"
            enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="bg-gray-200 p-6 rounded-md">
                <h2 class="font-bold mb-4">UBAH RANGKUMAN FIRMAN</h2>
                <div class="grid grid-cols-2 gap-x-10 gap-y-2" x-data="{
                    tipe_rangkuman: @json($rangkuman->tipe_rangkuman),
                    kategoriSermonsRequired: @json($rangkuman->tipe_rangkuman == 'Sermons')
                }" x-init="$watch('tipe_rangkuman', value => {
                    kategoriSermonsRequired = (value === 'Sermons');
                })">
                    <div>
                        <label class="block font-semibold mb-1">ID RANGKUMAN</label>
                        <input type="text" name="id_rangkuman" value="{{ $rangkuman->id_rangkuman_firman }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_narasumber" autocomplete="off"
                            placeholder="Masukkan Nama Narasumber" value="{{ $rangkuman->nama_narasumber }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="judul_rangkuman" autocomplete="off"
                            placeholder="Masukkan Judul Rangkuman" value="{{ $rangkuman->judul_rangkuman }}"
                            class="w-full p-2 rounded bg-white" required>
                    </div>
                    <div>
                            ...
                            <label for="gambar_rangkuman" id="gambar_rangkuman-label"
                                ...
                                @if (isset($rangkuman->gambar_rangkuman))
                                    <div class="text-black">
                                        {{ \Illuminate\Support\Str::after($rangkuman->gambar_rangkuman, 'gambarRangkuman/') }}
                                    </div>
                                @else
                        ...
                        <div class="flex items-center space-x-6">
                            <label class="flex items-center space-x-2">
                                <input type="radio" name="tipe_rangkuman" @click="tipe_rangkuman = 'Sermons'"
                                    ...
                                    value="Sermons" {{ $rangkuman->tipe_rangkuman === 'Sermons' ? 'checked' : '' }}
                                    required>
                                <span
                            ...
                            <label class="flex items-center space-x-2">
                                <input type="radio" name="tipe_rangkuman" @click="tipe_rangkuman = 'Articles'"
                                    ...
                                    {{ $rangkuman->tipe_rangkuman === 'Articles' ? 'checked' : '' }}>
                                <span
                    ...
                    <div>
                        ...
                        <select name="kategori_sermons" id="kategori_sermons"
                            ...
                            <option value="Sunday Service"
                                {{ old('kategori_sermons', $rangkuman->kategori_sermons ?? '') == 'Sunday Service' ? 'selected' : '' }}>
                                Sunday Service
                            </option>

                            <option value="Shabbat Fellowship"
                                {{ old('kategori_sermons', $rangkuman->kategori_sermons ?? '') == 'Shabbat Fellowship' ? 'selected' : '' }}>
                                Shabbat Fellowship
                            </option>
                        </select>
                    </div>
                ...
                <div class="mb-4">
                    ...
                    <input id="isi_rangkuman" type="hidden" name="isi_rangkuman"
                        value="{{ $rangkuman->isi_rangkuman }}" required>
